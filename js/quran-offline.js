...
const QuranOfflineService = {
  ...
  // Prioridad: MP3 de la sura completa en el CDN (un solo archivo). Si no
  // existe para ese recitador, se concatenan las aleyas ya cacheadas; los
  // MP3 MPEG-1 Layer III se pueden unir directamente.
  surahFileUrlFor(reciter, surahNumber) {
    return `https://cdn.islamic.network/quran/audio-surah/${this.AUDIO_BITRATE}/${reciter}/${surahNumber}.mp3`;
  },
  ...
  async exportSurahAudio(reciter, surahNumber) {
    ...
    try {
      // 1) Intento: archivo completo de la sura en el CDN
      if (navigator.onLine) {
        try {
          const res = await fetch(this.surahFileUrlFor(reciter, meta.number));
          if (res && res.ok) {
            const ct = (res.headers.get('content-type') || '').toLowerCase();
            const blob = await res.blob();
            if (blob.size > 1024 && (ct.includes('audio') || ct.includes('octet-stream') || blob.type.includes('audio') || blob.type === '')) {
              this._triggerBlobDownload(blob, name);
              return { ok: true, source: 'cdn', bytes: blob.size, name };
            }
          }
        } catch (e) { /* pasa al plan B */ }
      }
      // 2) Plan B: concatenar las aleyas cacheadas de esa sura
      if (this._audioCacheSupported()) {
        const cache = await caches.open(this.AUDIO_CACHE);
        const start = this._surahStartGlobal(meta.number);
        const parts = [];
        for (let i = 0; i < meta.numberOfAyahs; i++) {
          ...
          if (!hit) { parts.length = 0; break; } // falta alguna aleya: no exportable
