...
/* ============================================
   Upcoming Islamic Events Summary
   Renders cards for nearest upcoming events
   ============================================ */
const KEY_EVENTS = ['ramadan_start','laylat_qadr','eid_fitr','arafah','eid_adha','hijri_new_year','ashura','mawlid','isra_miraj','nisf_shaban'];

const EVENT_ICONS = {
    ramadan_start: 'fa-moon',
    laylat_qadr: 'fa-star',
    eid_fitr: 'fa-gift',
    arafah: 'fa-mountain',
    eid_adha: 'fa-kaaba',
    hijri_new_year: 'fa-calendar-plus',
    ashura: 'fa-droplet',
    mawlid: 'fa-heart',
    isra_miraj: 'fa-rocket',
    nisf_shaban: 'fa-moon'
};
...
async function renderUpcomingEvents() {
    ...
    // Find nearest upcoming date for each key event
    const found = {};
    const sortedDates = Object.keys(EVENTS_MAP).sort();
    ...
    if (!items.length) {
        grid.innerHTML = `<p class="upcoming-empty" style="text-align:center;color:#666;grid-column:1/-1;">No hay eventos próximos en los datos cargados.</p>`;
        return;
    }
    ...
    for (const it of items) {
        ...
        let hijriLabel = '';
        if (hijri) {
            const hMonthIdx = parseInt(hijri.month.number) - 1;
            hijriLabel = `${hijri.day} ${HIJRI_MONTHS[lang][hMonthIdx]} ${hijri.year} ${isAr ? 'هـ' : 'AH'}`;
        }
        ...
        card.className = `upcoming-card ${it.key}`;
        card.innerHTML = `
            ...
            <div class="up-card-body">
                ...
                <div class="up-card-dates">
                    ...
                    ${hijriLabel ? `<div class="up-date hijri"><i class="fas fa-moon"></i> ${hijriLabel}</div>` : ''}
                </div>
            </div>
        `;
...
// Trigger on load + language change
document.addEventListener('DOMContentLoaded', () => {
